...
import com.noto.app.ai.ApiClient
...
//        rv.edgeEffectFactory = BounceEdgeEffectFactory()
    ...
    private fun processImage(imageUri: android.net.Uri) {
        lifecycleScope.launch {
            try {
                // 显示加载状态
                Toast.makeText(context, "正在识别文字...", Toast.LENGTH_SHORT).show()

                // 将Uri转换为File
                val file = withContext(Dispatchers.IO) {
                    val inputStream = requireContext().contentResolver.openInputStream(imageUri)
                    val tempFile = File.createTempFile("ocr_", ".jpg", requireContext().cacheDir)
                ...
                val response = withContext(Dispatchers.IO) {
                    ApiClient.apiService.recognizeText(body)
                }

                if (response.success && response.text != null) {
                    // 将识别结果插入到笔记正文
                    val etBody = _binding?.etNoteBody ?: return@launch
                    val currentBody = etBody.text.toString()
                    val newBody = if (currentBody.isBlank()) {
                        response.text
                    } else {
                        "$currentBody\n\n${response.text}"
                    }
                    etBody.setText(newBody)
                    Toast.makeText(context, "文字识别成功", Toast.LENGTH_SHORT).show()
                } else {
                    Toast.makeText(context, response.message ?: "识别失败", Toast.LENGTH_SHORT).show()
                }
            } catch (e: Exception) {
