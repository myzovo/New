...
| 里程碑 | 时间 | 交付物 | 验证方式 |
|--------|------|--------|----------|
| M0 | 第2周末 | Spring Boot项目搭建完成，Noto可编译运行 | 后端能启动，App能运行 |
| M1 | 第4周末 | OCR功能端到端打通 | App拍照 → 后端识别 → 显示结果 |
| M2 | 第7周末 | 向量数据库集成，笔记自动向量化 | 保存笔记 → Supabase有向量记录 |
| M3 | 第10周末 | AI对话功能完成 | App提问 → 返回基于笔记的回答 |
| M4 | 第12周末 | 优化完成，可部署版本 | 完整流程无明显bug |
...
1. **立即执行**：创建Spring Boot项目，配置Supabase
2. **本周目标**：跑通后端OCR接口，App能调用成功
3. **持续迭代**：每周末验证一次里程碑，及时调整计划
